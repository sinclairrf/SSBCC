...
class SSBCCconfig():
  ...
  def IsStrobeOnlyOutport(self,outport):
    ...
    Note:  Peripherals can define outports with no attached signals which are
           interpreted as strobe-only outports, see for example big_inport.  The
           peripheral must define the output port behavior since it will not be
           generated by the normal module generation scripts.
    """
    if not outport[1:]:
      return True;
